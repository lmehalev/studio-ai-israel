...
const kpis = [
  ...
  { label: 'סרטונים שנוצרו', value: 12, icon: Video, color: 'text-primary' },
  { label: 'משימות פעילות', value: mockJobs.filter(j => j.status === 'בעיבוד' || j.status === 'ממתין').length, icon: Zap, color: 'text-warning' },
];

const quickActions = [
  { label: 'יצירת אווטאר חדש', icon: Users, path: '/avatars', color: 'from-emerald-600/20 to-emerald-600/5' },
  { label: 'יצירת סרטון חדש', icon: Video, path: '/create-video', color: 'from-primary/20 to-primary/5' },
  { label: 'העלאת מדיה', icon: Upload, path: '/avatars', color: 'from-blue-600/20 to-blue-600/5' },
    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-muted-foreground mt-1">הנה סיכום הפעילות שלך היום</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-lg font-rubik font-semibold">אווטארים אחרונים</h2>
                  ...
                  >
                    ...
                    <div className="mt-2">
                      <div className="w-full bg-muted rounded-full h-1.5">
                        <div className="bg-primary rounded-full h-1.5" style={{ width: `${avatar.qualityScore}%` }} />
                      </div>
                      <span className="text-[10px] text-muted-foreground">{avatar.qualityScore}% מוכנות</span>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          <div className="space-y-6">
            ...
            {/* Popular Templates */}
            <div className="bg-card border border-border rounded-xl p-5">
              <h3 className="font-rubik font-semibold mb-3">תבניות פופולריות</h3>
              <ul className="space-y-2">
                {mockTemplates.slice(0, 4).map(t => (
                  <Link key={t.id} to="/templates" className="flex items-center justify-between p-2 rounded-lg hover:bg-muted/50 transition-colors">
                    ...
                    <span className="text-xs text-muted-foreground">{t.usageCount} שימושים</span>
                  </Link>
                ))}
              </ul>
            </div>
