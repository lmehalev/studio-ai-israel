...
export default function AvatarsManagePage() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setCreating(false);
      setName(''); setPhotos([]); setStyle('professional headshot'); setExpression('neutral'); setBaseAvatarId('');
        ...
        {creating && (
          <div className="bg-card border border-border rounded-xl p-6 space-y-5">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { setCreating(false); setBaseAvatarId(''); setPhotos([]); }} className="p-1 hover:bg-muted rounded-lg">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <button onClick={handleCreate} disabled={generating} className="w-full gradient-gold text-primary-foreground py-3 rounded-lg font-semibold text-sm flex items-center justify-center gap-2 disabled:opacity-50">
